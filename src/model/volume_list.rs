...
mod imp {
    ...
    pub(crate) struct VolumeList {
        pub(super) list: RefCell<IndexMap<String, model::VolumeObject>>,
...
impl VolumeList {
    ...
    pub(crate) fn total_size(&self) -> u64 {
        self.imp()
            ...
            .borrow()
            .values()
            .map(model::VolumeObject::volume)
            .map(|volume| {
                volume
                    .usage_data
                    .as_ref()
                    .map(|data| data.size as u64)
                    .unwrap_or(0)
            })
            .sum()
    }

    pub(crate) fn get_volume<Q: Borrow<str> + ?Sized>(
        &self,
        name: &Q,
    ) -> Option<model::VolumeObject> {
        ...
        F: FnOnce(super::RefreshError) + Clone + 'static,
    {
        match event.action.as_str() {
            "remove" => self.remove_volume(&event.actor.id),
    ...
    pub(crate) fn connect_volume_added<F: Fn(&Self, &model::VolumeObject) + 'static>(
        &self,
        f: F,
